            ...
            <div class="col-md-6">
                    ...
                    <div class="form-group">
                        ...
                        <div class="">
                            <select class="form-control form-control-sm" name="media" id="mediaSelect">
                                ...
                                <?php 
                                    foreach ($dataConn as $item): 
                                        if($item->MediaId == 'WHATP'): $mediaName = 'Whatsapp';endif;
                                        if($item->MediaId == 'EMAIL'): $mediaName = 'Email';endif;?>
                    ...
                    <div class="form-group">
                        ...
                        <div class="">
                            ...
                            <small class="text-italic">Ini hanya untuk penjadwalan pesan, jika ingin langsung dikirim tidak perlu di isi</small>
                        </div>
                    </div>
                    ...
                    <div class="">
                        <select class="form-control form-control-sm m-b-none" name="messageTemplate" id="templateMessage">
                            ...
                            <?php foreach ($dataTemplate as $item): ?>
                                ...
                                <option value="<?= $item->Id ?>" data-mediaid="<?= $item->MediaId ?>" data-typeid="<?= $item->TypeId ?>" data-content="<?= htmlspecialchars($item->Content) ?>" data-params="<?= implode(', ', $metaData['params']) ?>" data-meta='<?= $item->Meta?>'>
                                    <?= $item->Title ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
...
<script>
    ...
    $('form').submit(function (e) {
        ...
        var typeTemplate = $('select[name="messageTemplate"] option:selected').data('typeid');
        var metaTemplate = $('select[name="messageTemplate"] option:selected').data('meta');
        // var fixParamTemplate = $('select[name="messageTemplate"] option:selected').data('fixparam');
        var variableMessage = $('input[name="variableMessage"]').val();
        var message = $('textarea[name="message"]').val();
        var convertedVariable = replaceVarMessage(message, variableMessage);
        var toContactWrap = toContact.join(", ");
        var toGroupContactWrap = toGroupContact.join(", ");
        // Susun objek data
        var formData = {
            MediaId: mediaId,
            From: from,
            ...
            ToInput: toInput,
            ToContact: toContactWrap,
            ToGroupContact: toGroupContactWrap,
            SendDate: sendDate,
            Delay: delay,
            Subject: subject,
            ...
            // FixParamTemplate: fixParamTemplate,
    ...
    $('#toContact').on('change', function() {
        var selectAll = $('#toContact option[value="selectAll"]');
        var allOptions = $('#toContact option:not([value="selectAll"])');
        
        if (selectAll.is(':selected')) {
            allOptions.prop('selected', true);
            selectAll.prop('selected', false); // Deselect "Select All" after selecting all others
        } else {
            // Check if all other options are selected
            var allSelected = allOptions.length === allOptions.filter(':selected').length;
            if (!allSelected) {
                selectAll.prop('selected', false); // Deselect "Select All" if not all are selected
            }
        }

        $('#toContact').trigger('change.select2'); // Trigger change event for Select2
    });

    $('#toContactGroup').on('change', function() {
        var selectAll = $('#toContactGroup option[value="selectAll"]');
        var allOptions = $('#toContactGroup option:not([value="selectAll"])');
        ...
        $('#toContactGroup').trigger('change.select2'); // Trigger change event for Select2
    });
</script>